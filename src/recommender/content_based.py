...
class ContentBasedModel:
    ...
    def predict_rating(self, user_id, movie_id, top_k=20):
        ...
        # We need similarities between Target Movie and ALL movies in History
        ...
        # Get Vectors
        # target_vec: (1, n_features)
        # history_vecs: (n_history, n_features)
        ...
        # Remove negative similarities? (Optional, usually keep for regression or clip at 0)
        # Literature: Usually keep 0-1 for content. TF-IDF is non-negative, so Cosine is usually >=0 unless centered.
        # Sklearn cosine on TF-IDF is [0, 1].
    ...
    def get_explanation(self, user_id, movie_id, top_k=5):
        ...
        # 1. Get feature contribution
        # We can't easily decompose the dot product from the profile centroid without re-calculating per-feature.
        # Instead, we look at the User's TOP rated movies and find the commonalities with the Candidate.
        ...
        if not liked_movies:
             liked_movies = user_ratings['movieId'].tolist() # Fallback
             
        # Find the most similar liked movie to the candidate
        # We can use the text soup similarity
        
        target_idx = self.movie_to_idx.get(movie_id)
        if target_idx is None:
            return {"score": 0, "features": []}
            
        # Brute force check similarity against top 20 recent liked movies for speed
        ...
        # Also check direct profile match (Top Global Keywords)
        # (Simplified to just return the pair explanation as it's more concrete)
    ...
    def get_shared_traits(self, movie_a_id, movie_b_id):
        ...
        def get_vals(mid, col):
            try:
                ...
                # Clean splitter: handles both '|' and ',' and strips whitespace
    ...
    def _explain_pair(self, movie_a, movie_b):
        """
        Legacy helper for internal string formatting.
        Now uses get_shared_traits.
        """
